    ...
    <div className="space-y-4 sm:space-y-8">
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
        ...
        <div className="p-3 sm:p-6">
          {/* Day Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 sm:mb-6 pb-3 sm:pb-4 border-b border-slate-100 gap-3">
            <div>
              <h3 className="text-lg sm:text-2xl font-black text-slate-900">{currentDay.title || `Day ${currentDay.day}`}</h3>
              <p className="text-slate-500 text-xs sm:text-sm mt-1">{currentDay.activities?.length || 0} activities planned</p>
            </div>
            <div className="text-left sm:text-right">
              <p className="text-[10px] sm:text-xs text-slate-400 font-bold uppercase tracking-wider">Day Budget</p>
              <p className="text-xl sm:text-2xl font-black text-blue-600">{formatMoney(getDayCost(currentDay).total, getDayCost(currentDay).symbol)}</p>
              <p className="text-[8px] sm:text-[9px] text-blue-400 font-bold uppercase tracking-wider"><i className="fas fa-user mr-1"></i>Per Person</p>
            </div>
          </div>
        ...
        {/* Day Navigation */}
        <div className="flex items-center justify-between p-2 sm:p-4 bg-slate-50 border-t border-slate-100">
          <button
            disabled={activeDay === 0}
            onClick={() => setActiveDay(prev => prev - 1)}
            className="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 text-slate-600 hover:text-blue-600 disabled:opacity-30 disabled:pointer-events-none transition-all text-xs sm:text-base"
          >
            <i className="fas fa-arrow-left text-xs sm:text-base"></i>
            <span className="font-bold hidden sm:inline">Previous Day</span>
            <span className="font-bold sm:hidden">Prev</span>
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 sm:gap-2 px-3 py-1.5 sm:px-4 sm:py-2 text-slate-600 hover:text-blue-600 disabled:opacity-30 disabled:pointer-events-none transition-all text-xs sm:text-base"
          >
            <span className="font-bold hidden sm:inline">Next Day</span>
            <span className="font-bold sm:hidden">Next</span>
            <i className="fas fa-arrow-right text-xs sm:text-base"></i>
          </button>
        </div>
      </div>
...
const DayCard: React.FC<{ day: DayPlan; destination: string; isExpanded: boolean; onToggle: () => void }> = ({ day, destination, isExpanded, onToggle }) => {
  ...
  return (
    <div className={`bg-white rounded-xl sm:rounded-2xl border transition-all ${isExpanded ? 'border-blue-200 shadow-lg' : 'border-slate-100 shadow-sm hover:shadow-md'}`}>
      {/* Header */}
      <button
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-2 sm:gap-4">
          <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center font-black text-base sm:text-lg ${isExpanded ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600'}`}>
            {day.day}
          </div>
          <div>
            <h4 className="font-bold text-slate-900 text-sm sm:text-base">{day.title || `Day ${day.day}`}</h4>
            <p className="text-xs sm:text-sm text-slate-500">{day.activities?.length || 0} activities</p>
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <i className={`fas fa-chevron-down text-slate-400 transition-transform text-sm ${isExpanded ? 'rotate-180' : ''}`}></i>
        </div>
      </button>

      {/* Expanded Content */}
      {isExpanded && (
        <div className="px-2 sm:px-4 pb-3 sm:pb-4 space-y-1.5 sm:space-y-2 border-t border-slate-100 pt-3 sm:pt-4">
          {day.activities?.map((activity, idx) => (
            <div key={idx} className="flex items-center gap-2 sm:gap-3 py-1.5 sm:py-2 px-2 sm:px-3 bg-slate-50 rounded-lg sm:rounded-xl">
              <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-md sm:rounded-lg flex items-center justify-center ${getCategoryBg(activity.category)}`}>
                <i className={`fas ${getActivityIcon(activity.category)} text-xs sm:text-sm`}></i>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-slate-900 text-xs sm:text-sm truncate">{activity.location || activity.wikipediaTitle || 'Activity'}</p>
                <p className="text-[10px] sm:text-xs text-slate-500">{activity.time || 'Flexible timing'}</p>
              </div>
              <span className={`text-[10px] sm:text-xs font-bold px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md sm:rounded-lg whitespace-nowrap ${isFreeCost(activity.costEstimate) ? 'bg-emerald-100 text-emerald-700' : 'bg-white text-slate-700 border border-slate-200'}`}>
...
const ActivityRow: React.FC<{ activity: Activity; index: number; destination: string }> = ({ activity, index, destination }) => {
  ...
  return (
    <div className="group flex items-center gap-2 sm:gap-4 p-2 sm:p-3 rounded-xl sm:rounded-2xl hover:bg-slate-50 transition-all">
      {/* Image Thumbnail */}
      <div className="flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded-lg sm:rounded-xl overflow-hidden bg-slate-100">
        <img
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1 sm:gap-2 mb-0.5 sm:mb-1 flex-wrap">
          <span className="text-[10px] sm:text-xs font-bold text-blue-600 bg-blue-50 px-1.5 sm:px-2 py-0.5 rounded-full">
            {activity.time || `Stop ${index + 1}`}
          </span>
          <span className={`text-[10px] sm:text-xs font-bold px-1.5 sm:px-2 py-0.5 rounded-full ${getCategoryBg(activity.category)}`}>
            <i className={`fas ${getActivityIcon(activity.category)} mr-1`}></i>
            <span className="hidden sm:inline">{activity.category || 'Visit'}</span>
          </span>
        </div>
        <h4 className="font-bold text-slate-900 truncate text-sm sm:text-base">{title}</h4>
        <p className="text-[10px] sm:text-xs text-slate-500 line-clamp-1 hidden sm:block">{activity.description || `Experience ${title}`}</p>
      </div>

      {/* Cost */}
      <div className="flex-shrink-0 text-right">
        <span className={`inline-flex items-center gap-0.5 sm:gap-1 px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-bold ${
          isFree ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-900'
        }`}>
          {isFree ? <i className="fas fa-gift text-xs"></i> : <i className="fas fa-tag text-blue-500 text-xs"></i>}
          <span className="hidden sm:inline">{activity.costEstimate || 'Free'}</span>
          <span className="sm:hidden">{(activity.costEstimate || 'Free').replace(/per person/i, '').trim()}</span>
        </span>
      </div>
    </div>
  );
